...
def get_federated_datasets(
  ...
  train, test = tff.simulation.datasets.cifar100.load_data(
      cache_dir='/storage-6T/kane/cifar100/')
 
  json_path = '/storage-6T/kane/cifar100/'
  train_path = json_path + 'southwest_images_new_test.pkl'
  test_path = json_path + 'southwest_images_new_test.pkl'
  train_objects = get_objects(train_path)
  test_objects = get_objects(test_path)

  def get_user_data_train(client_id):
    labels = np.array([9 for _ in train_objects[0]])
    images = np.array(train_objects[0])
    return tf.data.Dataset.from_tensor_slices(({
      'image': images,
      'label': labels}))
  
  def get_user_data_test(client_id):
    labels = np.array([9 for _ in test_objects[0]])
    images = np.array(test_objects[0])
    return tf.data.Dataset.from_tensor_slices(({
      'image': images,
      'label': labels}))

  mal_clients = [666]
  mal_train_clientdata = tff.simulation.datasets.ClientData.from_clients_and_fn(
      mal_clients,
      get_user_data_train)
  
  mal_clients_test = [666]
  mal_test_clientdata = tff.simulation.datasets.ClientData.from_clients_and_fn(
      mal_clients_test,
      get_user_data_test)
  ...
  mal_train_preprocess_fn = create_preprocess_fn(
      num_epochs=train_client_epochs_per_round,
      batch_size=train_client_batch_size,
      shuffle_buffer_size=train_shuffle_buffer_size,
      crop_shape=crop_shape,
  ...
  mal_test_preprocess_fn = create_preprocess_fn(
      num_epochs=test_client_epochs_per_round,
      batch_size=test_client_batch_size,
      shuffle_buffer_size=test_shuffle_buffer_size,
      crop_shape=crop_shape,
  ...
  train = train.preprocess(train_preprocess_fn)
  test = test.preprocess(test_preprocess_fn)
  mal_train = mal_train_clientdata.preprocess(mal_train_preprocess_fn)
  mal_test = mal_test_clientdata.preprocess(mal_test_preprocess_fn)
  return train, test, mal_train, mal_test
